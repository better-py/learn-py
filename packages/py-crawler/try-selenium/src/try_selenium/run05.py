...
#
LOGIN_GB_URL = "https://ais.usvisa-info.com/en-gb/niv/users/sign_in"
...
def getInfo():
    ...
    response = session.get(LOGIN_CA_URL, headers=headers).text
    ...
    print(f"http header csrf-token: {auth_token}")

    return auth_token, preSession, USER_AGENT
...
def login(email, password):
    ...
    loginResponse = loginSession.post(LOGIN_CA_URL, headers=headers, data=payload)

    print(f"login response: {loginResponse.text}")

    loginSessionName = ''
    loginSessionValue = ''

    print(f"cookie: {loginSession.cookies}")
...
def execute():
    ...
    scheduleID = os.getenv("SCHEDULEID")
    ...
    expectedDate = os.getenv("EXPECTEDDATE") or "2024-07-10"

    if not email or not password or not scheduleID:
        print("Please set EMAIL, PASSWORD, and SCHEDULEID environment variables")
        return

    print(f"email: {email}, password: {password}, scheduleID: {scheduleID}, expectedDate: {expectedDate}")
    ...
    while True:
        ...
        try:
            ...
            spot1_resp = londonSession.get(get_schedule_api("ca", scheduleID, "95"), headers=headers)
            spot2_resp = requests.get(get_schedule_api("ca", scheduleID, "94"), headers=headers)

            #
            #
            #

            print(f"spot 1 response: {spot1_resp.text}")
            print(f"spot 2 response: {spot2_resp.text}")

            print("时间: " + str(time.asctime()))

            # london slot check
            earliestLondon = spot1_resp.text[10:20]
            print('London:')
            if len(spot1_resp.text) == 2:
                print("呜呜呜伦敦slot都没有啦")
                ...
                #
                #
                # todo x: send notification here!
                print(f"Sending notification: {mailContent}")

            else:
                retry()

            # belfast slot check
            earliestBelfast = spot2_resp.text[10:20]
            print('Belfast:')
            if len(spot2_resp.text) == 2:
                print("呜呜呜贝法slot都没有啦\n")
            ...
            counter = counter + 1
            if counter == 10:
                londonCookie = spot1_resp.cookies
